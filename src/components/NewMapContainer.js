...
export class NewMapContainer extends Component {
	...
	fetchNearby = () => {
		let urlWithTypestr = `https://maps.googleapis.com/maps/api/place/radarsearch/json?location=${
			this.state.lat
		},${this.state.lng}&radius=1000&type=${this.props.searchStr}&keyword=${
			this.props.typeStr
		}&key=AIzaSyAF3laRwdxS7LqBHaCP5UbQX-ZKOOTFPwE`;
		let urlWithoutTypestr = `https://maps.googleapis.com/maps/api/place/radarsearch/json?location=${
			this.state.lat
		},${this.state.lng}&radius=1000&type=${
	...
	fetchPlacesInfo = () => {
		...
		this.state.markerPostions.forEach(
			x =>
				fetch(
	...
	render() {
		...
		return (
          ...
          <Sidedock
            placesData={this.placeData}
            count={this.state.markerPostions.length}
